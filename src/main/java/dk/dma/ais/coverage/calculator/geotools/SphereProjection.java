...
public class SphereProjection {
    ...
    public double lon2x(double lon, double lat) {
        ...
        double x = 0.0;
        double denom = (1.0 + Math.sin(lat0_rad) * Math.sin(lat_rad) + Math.cos(lat0_rad) * Math.cos(lat_rad)
                * Math.cos(lon_rad - lon0_rad));
    ...
    public double lat2y(double lon, double lat) {
        ...
        if (denom != 0.0) {
            y = ((2.0 * radius) / denom)
                    * (Math.cos(lat0_rad) * Math.sin(lat_rad) - Math.sin(lat0_rad) * Math.cos(lat_rad)
                            * Math.cos(lon_rad - lon0_rad));
        }
