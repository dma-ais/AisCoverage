...
public class ChartGenerator {
    ...
    public void generateChartMethod2(Date startTime, Date endTime, List<TimeSpan> timeSpans, double latMin, double latMax,
            double lonMin, double lonMax, boolean use3d) {
        ...
        for (TimeSpan timeSpan : timeSpans) {
            if (timeSpan.getMessageCounterSat() > maxheight) {
                maxheight = timeSpan.getMessageCounterSat();
            }
            if (timeSpan.getMessageCounterTerrestrialUnfiltered() > maxheight) {
                maxheight = timeSpan.getMessageCounterTerrestrialUnfiltered();
            }
        }
        // double scale = 1;
        ...
        for (int i = 0; i < timeDifference; i++) {
            ...
            if (i % 3 == 0) {
                ...
                ig2.setPaint(Color.LIGHT_GRAY);
                // dateLabel = ('0' + currentDate.getDate()).slice(-2)+"-"+('0' + currentDate.getMonth()).slice(-2)+" "+(('0' +
                // currentDate.getHours()).slice(-2)+":00");
        ...
        for (TimeSpan timeSpan : timeSpans) {
            ...
            ig2.drawString(totalDistinctShips + "", x + 30 - getTextWidth(totalDistinctShips + "") / 2, height - 1 - bottomOffset
                    + 15);
            ig2.drawString(timeSpan.getDistinctShipsTerrestrial().size() + "", x + 30
                    - getTextWidth(timeSpan.getDistinctShipsTerrestrial().size() + "") / 2, height - 1 - bottomOffset + 30);
            ig2.drawString(timeSpan.getDistinctShipsSat().size() + "", x + 30
                    - getTextWidth(timeSpan.getDistinctShipsSat().size() + "") / 2, height - 1 - bottomOffset + 45);
        ...
        ig2.drawString("Distinct Ships - Terrestrial", offset - getTextWidth("Distinct Ships- Terrestrial"), height - 1
                - bottomOffset + 30);
        ig2.drawString("Distinct Ships - Satellite", offset - getTextWidth("Distinct Ships- Satellite"), height - 1 - bottomOffset
                + 45);
...
//        System.out.println((double) (Math.round((double) minutesCovered / exactTimeDifference * 10000)) / 100);
        ...
        // ig2.drawString(maxBetweenBarWidth+" min", descriptionWidth-getTextWidth(maxBetweenBarWidth+" min"),
        // descriptionTopOffset);
        //
        // descriptionTopOffset+=20;
        // ig2.drawString("Avg. \"hole\" width: ", 10, descriptionTopOffset);
        // ig2.drawString(avgBetweenBarWidth+" min", descriptionWidth-getTextWidth(avgBetweenBarWidth+" min"),
        // descriptionTopOffset);
        //
        // descriptionTopOffset+=20;
    ...
    public void generateChartMethod1(Date startTime, Date endTime, List<TimeSpan> timeSpans, double latMin, double latMax,
        ...
        for (TimeSpan timeSpan : timeSpans) {
            ...
            // Draw counter label
            ig2.setColor(Color.black);
            int stringLen = (int) ig2
                    .getFontMetrics()
                    .getStringBounds("" + timeSpan.getMessageCounterSat() + " (" + timeSpan.getDistinctShipsSat().size() + ")", ig2)
                    .getWidth();
            ig2.drawString("" + timeSpan.getMessageCounterSat() + " (" + timeSpan.getDistinctShipsSat().size() + ")", x
                    + (barwidth / 2) - (stringLen / 2), y - 5);
    ...
    public static void main(String[] args) throws Exception {
        ...
        cg.generateChartMethod2(s1.getFirstMessage(), new Date(s2.getLastMessage().getTime() + 1000 * 60 * 60 * 8), l, 10.234,
                12.342, 55.7453, 56.02938, true);
    ...
    public void generateChartMethod3(Date startTime, Date endTime, int shipMMsi, List<ExportShipTimeSpan> timeSpans, boolean b) {
        if (timeSpans.isEmpty()) {
            bi = new BufferedImage(200, 100, BufferedImage.TYPE_INT_ARGB);
            ig2 = bi.createGraphics();
            ig2.setPaint(Color.black);
        ...
        for (ExportShipTimeSpan timeSpan : timeSpans) {
            ...
            // ig2.setColor(Color.black);
            // int stringLen = (int)
            // ig2.getFontMetrics().getStringBounds(""+timeSpan.getMessageCounterSat()+" ("+timeSpan.getDistinctShipsSat().size()+")",
            // ig2).getWidth();
